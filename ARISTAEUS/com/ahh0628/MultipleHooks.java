...
public class MultipleHooks {
    ...
    public static void make() throws IOException, InterruptedException {
        try {
            //WEBHOOK ONE
                            ...
                            case "y":
                                case "Y":
                                    if (hook1HasMessage) {
                                        ...
                                        System.out.println("Webhook 1 didn't have message, skipping..");
                                    }
                                    if (hook2HasMessage) {
                                        REF.webhook2.execute();
                                        TimeUnit.MILLISECONDS.sleep(400);
                                    } else {
                                        System.out.println("Webhook 2 didn't have message, skipping..");
                                    }
                                    if (hook3HasMessage) {
                                        REF.webhook3.execute();
                                        TimeUnit.MILLISECONDS.sleep(400);
                                    } else {
                                        System.out.println("Webhook 3 didn't have message, skipping..");
                                    }
                                    if (hook4HasMessage) {
                                        REF.webhook4.execute();
                                        TimeUnit.MILLISECONDS.sleep(400);
                                    } else {
                                        System.out.println("Webhook 4 didn't have message, skipping..");
                                    }
                                    if (hook5HasMessage) {
                                        REF.webhook5.execute();
                                        TimeUnit.MILLISECONDS.sleep(400);
                                    } else {
                                        System.out.println("Webhook 5 didn't have message, skipping..");
                                    }
                                    break;
                                    default:
                                        System.exit(0);
